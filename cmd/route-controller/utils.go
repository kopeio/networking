...
func (t *taskQueue) worker() {
	for {
		...
		glog.V(3).Infof("syncing %v", keyString)
		err := t.sync(keyString)
		if err != nil {
			glog.Warningf("unexpected error processing messsage %q, sleeping before retry: %v", keyString, err)
			time.Sleep(retryDelay)
			t.requeue(keyString, err)
		}
		t.queue.Done(key)
	}
}
